...
function _inlineFragments(node, fragments) {
    ...
    return (0, visitWithMemo_js_1.visitWithMemo)(node, {
        [graphql_1.Kind.SELECTION_SET]: (selectionSetNode) => processSelectionSet(selectionSetNode, fragmentMap),
    }, {
...
function processSelectionSet(selectionSet, fragmentMap, visitedFragments = new Set()) {
    return (0, updateNode_js_1.updateNode)(selectionSet, 'selections', processSelections(selectionSet.selections, fragmentMap, visitedFragments));
}
function processSelections(fragmentSelections, fragmentMap, visitedFragments) {
    ...
    for (const selection of fragmentSelections) {
        const newSelection = processSelection(selection, fragmentMap, visitedFragments);
        if (newSelection !== null) {
            newSelections = (0, appendToArray_js_1.appendToArray)(newSelections, newSelection);
        }
    }
...
function toInlineFragment(spreadNode, fragmentMap, visitedFragments) {
    ...
    let newNode = (0, updateNode_js_1.updateNode)(spreadNode, 'kind', graphql_1.Kind.INLINE_FRAGMENT);
    newNode = (0, updateNode_js_1.updateNode)(newNode, 'name', null);
    newNode = (0, updateNode_js_1.updateNode)(newNode, 'typeCondition', fragment.typeCondition);
    newNode = (0, updateNode_js_1.updateNode)(newNode, 'selectionSet', processSelectionSet(fragment.selectionSet, fragmentMap, visitedFragments));
    return newNode;
}
