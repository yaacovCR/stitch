...
exports.visitWithMemo = visitWithMemo;
...
function visitWithMemo(root, visitor, visitorKeys = DocumentKeys) {
    const enterLeaveMap = new Map();
    for (const kind of Object.values(graphql_1.Kind)) {
    ...
    do {
        ...
        if (isLeaving) {
            ...
            if (isEdited) {
                if (inArray) {
                    let arr = appendToArray_js_1.emptyArray;
                    for (let arrIndex = 0; arrIndex < node.length; arrIndex++) {
                        const editValue = edits.get(arrIndex);
                        if (editValue === undefined) {
                            arr = (0, appendToArray_js_1.appendToArray)(arr, node[arrIndex]);
                        }
                        else if (editValue !== null) {
                            arr = (0, appendToArray_js_1.appendToArray)(arr, editValue);
                        }
                    }
                    node = arr;
                }
                    ...
                    for (const [editKey, editValue] of edits) {
                        newNode = (0, updateNode_js_1.updateNode)(newNode, editKey, editValue);
                    }
                    node = newNode;
                }
            }
