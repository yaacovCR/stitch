...
function handleMaybeAsyncResult(context, stitch, initialResult) {
    ...
    const promise = initialResult.then((resolved) => handleResult(context, stitch, resolved), (err) => handleResult(context, stitch, {
        data: null,
        errors: [
            new graphql_1.GraphQLError(err.message, {
                originalError: err,
            }),
        ],
    }));
...
function collectStitches(context, stitchMap, pointer, fieldPlansByType) {
    ...
    (typeName != null) || (0, invariant_js_1.invariant)(false, `Missing entry '__stitching__typename' in response ${(0, inspect_js_1.inspect)(target)}.`);
    const type = context.superSchema.getType(typeName);
    ((0, graphql_1.isObjectType)(type)) || (0, invariant_js_1.invariant)(false, `Expected Object type, received '${typeName}'.`);
    const fieldPlan = fieldPlansByType.get(type);
    (fieldPlan !== undefined) || (0, invariant_js_1.invariant)(false, `Missing field plan for type '${typeName}'.`);
