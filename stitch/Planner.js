...
class Planner {
    constructor(superSchema, operation) {
        this._createRootPlan = (0, memoize2_js_1.memoize2)(this._createRootPlanImpl.bind(this));
        this._createFieldPlan = (0, memoize3_js_1.memoize3)(this._createFieldPlanImpl.bind(this));
        this._collectSubFields = (0, memoize2_js_1.memoize2)(this._collectSubFieldsImpl.bind(this));
    ...
    createRootPlan(variableValues) {
        ...
        const filteredOperation = (0, applySkipIncludeDirectives_js_1.applySkipIncludeDirectives)(this.operation, variableValues);
        const fieldNodes = this._collectSubFields(rootType, filteredOperation.selectionSet.selections);
        return this._createRootPlan(rootType, fieldNodes);
    }
    _collectSubFieldsImpl(runtimeType, selections, fieldNodes = appendToArray_js_1.emptyArray, visitedFragmentNames = new Set()) {
        let newFieldNodes = fieldNodes;
        const schema = this.superSchema.mergedSchema;
        for (const selection of selections) {
            switch (selection.kind) {
                case graphql_1.Kind.FIELD: {
                    newFieldNodes = (0, appendToArray_js_1.appendToArray)(newFieldNodes, selection);
                    break;
                }
                case graphql_1.Kind.INLINE_FRAGMENT: {
                    if (!this._doesFragmentConditionMatch(schema, selection, runtimeType)) {
                        continue;
                    }
                    newFieldNodes = this._collectSubFieldsImpl(runtimeType, selection.selectionSet.selections, newFieldNodes, visitedFragmentNames);
                    break;
                }
    ...
    _createFieldPlanImpl(parentType, fieldNodes, fromSubschema) {
        ...
        for (const fieldNode of fieldNodes) {
            this._addFieldToFieldPlan(fieldPlan, fromSubschema, parentType, fieldNode);
        }
        return {
            superSchema: fieldPlan.superSchema,
            subschemaPlans: [...fieldPlan.subschemaPlans.values()],
    ...
    _addFieldToFieldPlan(fieldPlan, fromSubschema, parentType, field) {
        const subschemaSetsByField = this.superSchema.subschemaSetsByTypeAndField[parentType.name];
        ...
        if (!field.selectionSet) {
            const { subschemaPlan } = this._getSubschemaAndPlan(subschemas, subschemaPlans, fromSubschema);
            subschemaPlan.fieldNodes = (0, appendToArray_js_1.appendToArray)(subschemaPlan.fieldNodes, field);
            return;
        }
        ...
        const selectionSplit = this._createSelectionSplit(namedFieldType, field.selectionSet.selections, subschema, fromSubschema);
        const fieldPlansByType = this._createStitchPlan(namedFieldType, selectionSplit.otherSelections, subschema);
        if (selectionSplit.ownSelections.length) {
            const subschemaPlan = this._getSubschemaPlan(subschema, subschemaPlans, fromSubschema);
            const splitField = {
                ...field,
                selectionSet: {
            ...
            subschemaPlan.fieldNodes = (0, appendToArray_js_1.appendToArray)(subschemaPlan.fieldNodes, splitField);
    ...
    _createStitchPlan(parentType, otherSelections, subschema) {
        ...
        for (const type of possibleTypes) {
            ...
            if (fieldPlan.subschemaPlans.length > 0 ||
    ...
    _createSelectionSplit(parentType, selections, subschema, fromSubschema) {
        ...
        this._processSelectionsForSelectionSplit(selectionSplit, subschema, fromSubschema, parentType, selections);
        if (fromSubschema === undefined &&
    ...
    _processSelectionsForSelectionSplit(selectionSplit, subschema, fromSubschema, parentType, selections) {
        for (const selection of selections) {
            switch (selection.kind) {
                case graphql_1.Kind.FIELD: {
                    this._addFieldToSelectionSplit(selectionSplit, subschema, fromSubschema, parentType, selection);
                    break;
                }
                case graphql_1.Kind.INLINE_FRAGMENT: {
                    const typeName = selection.typeCondition?.name.value;
                    const refinedType = typeName !== undefined
                        ? this.superSchema.getType(typeName)
                        : parentType;
                    ((0, graphql_1.isCompositeType)(refinedType)) || (0, invariant_js_1.invariant)(false, `Invalid type condition ${(0, inspect_js_1.inspect)(refinedType)}`);
                    this._addFragmentToSelectionSplit(selectionSplit, subschema, fromSubschema, refinedType, selection);
                    break;
                }
                case graphql_1.Kind.FRAGMENT_SPREAD: {
                    throw new Error('Unexpected fragment spread in selection set.');
    ...
    _addFieldToSelectionSplit(selectionSplit, subschema, fromSubschema, parentType, field) {
        const subschemaSetsByField = this.superSchema.subschemaSetsByTypeAndField[parentType.name];
        ...
        if (!field.selectionSet) {
            if (subschemaSet.has(subschema)) {
                selectionSplit.ownSelections = (0, appendToArray_js_1.appendToArray)(selectionSplit.ownSelections, field);
            }
            else {
                selectionSplit.otherSelections = (0, appendToArray_js_1.appendToArray)(selectionSplit.otherSelections, field);
            }
            return;
        }
        const fieldName = field.name.value;
        ...
        const subSelectionSplit = this._createSelectionSplit((0, graphql_1.getNamedType)(fieldType), field.selectionSet.selections, subschema, fromSubschema);
        ...
        if (subSelectionSplit.otherSelections.length) {
            selectionSplit.otherSelections = (0, appendToArray_js_1.appendToArray)(selectionSplit.otherSelections, {
                ...field,
                selectionSet: {
                    kind: graphql_1.Kind.SELECTION_SET,
    ...
    _addFragmentToSelectionSplit(selectionSplit, subschema, fromSubschema, parentType, fragment) {
        const fragmentSelectionSplit = {
            ownSelections: appendToArray_js_1.emptyArray,
            otherSelections: appendToArray_js_1.emptyArray,
        };
        this._processSelectionsForSelectionSplit(fragmentSelectionSplit, subschema, fromSubschema, parentType, fragment.selectionSet.selections);
        if (fragmentSelectionSplit.ownSelections.length > 0) {
            ...
            selectionSplit.ownSelections = (0, appendToArray_js_1.appendToArray)(selectionSplit.ownSelections, splitFragment);
        }
        if (fragmentSelectionSplit.otherSelections.length > 0) {
            ...
            selectionSplit.otherSelections = (0, appendToArray_js_1.appendToArray)(selectionSplit.otherSelections, splitFragment);
        }
    }
}
