...
import { executeSyncWithGraphQL as executeSync } from './execute.js';
...
describe('Execute: Handles inputs', () => {
      ...
      it('errors on null for nested non-null', () => {
        ...
        expectJSON(result).toDeepEqual({
          errors: [
            {
              message:
                'Variable "$input" has invalid value.c: Expected value of non-null type "String!" not to be null.',
      ...
      it('errors on deep nested errors and with many errors', () => {
        ...
        expectJSON(result).toDeepEqual({
          errors: [
            {
              message:
                'Variable "$input" has invalid value.na: Expected value of type "TestInputObject" to include required field "c", found: { a: "foo" }.',
  ...
  describe('Handles non-nullable scalars', () => {
    it('allows non-nullable variable to be omitted given a default', () => {
      ...
      expect(result).to.deep.equal({
        data: { fieldWithNullableStringInput: '"default"' },
      });
    });
    ...
    it('does not allow lists of non-nulls to contain null', () => {
      ...
      expectJSON(result).toDeepEqual({
        errors: [
          {
            message:
              'Variable "$input" has invalid value[1]: Expected value of non-null type "String!" not to be null.',
            locations: [{ line: 2, column: 16 }],
          },
        ],
      });
