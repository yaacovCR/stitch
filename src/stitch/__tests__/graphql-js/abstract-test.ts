...
import {
  executeSyncWithGraphQL as executeSync,
  executeWithGraphQL as execute,
} from './execute.js';
...
describe('Execute: Handles execution of abstract types', () => {
  ...
  it('isTypeOf can return false', async () => {
    ...
    const schema = new GraphQLSchema({
      query: new GraphQLObjectType({
        name: 'Query',
        fields: { pet: { type: PetType, resolve: () => ({}) } },
      }),
      types: [DogType],
    });
  ...
  it('resolveType can throw', async () => {
    // TODO: type context when generic parameters are released
    const PetType = new GraphQLInterfaceType({
      name: 'Pet',
  ...
  it('resolveType on Interface yields useful error', () => {
      ...
      return {
        toEqual(message: string) {
          expectJSON(result).toDeepEqual({
            data: { pet: null },
            errors: [
              { message, locations: [{ line: 3, column: 9 }], path: ['pet'] },
            ],
          });
        },
      };
    ...
    assertInterfaceType(schema.getType('Pet')).resolveType = () =>
      schema.getType('Cat');
    expectError({ forTypeName: undefined }).toEqual(
      'Abstract type "Pet" must resolve to an Object type at runtime for field "Query.pet" with value { __typename: undefined }, received "Cat", which is not a valid Object type name.',
    );
  });
});
